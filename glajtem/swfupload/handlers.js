...
function fileQueueError(file, errorCode, message) {
	try {
	   ...
	   if(swf_test) alert('Error Code: ' + errorCode);
		...
		if(swf_test) alert('addImage - ERROR(2)): ' + imageName);
	...
	try {
		...
		if(serverData.substring(0, 7) === "FILEID:")
		{
		   ...
		   //alert('FILEID:' + serverData.substring(7).split('#',1));
			...
			// -bo home.pl dokleja do tego jakieś swoje skrypty !!! 2013-02-08
...
function uploadComplete(file) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.setStatus("Wszystkie zdjęcia pobrane."); //All images received
			progress.toggleCancel(false);
		}
	} catch (ex) {
		this.debug(ex);
...
function addImage(src) {
	...
	if(swf_test) alert('addImage: ' + src);
   ...
   newImg.style.margin = "1em auto";
	...
	var div = '<div id=\'temp\' class=\'swf_formPlace\'>'+"\n";
		 ...
		 div += '<div><label for=\'tyt\'>tytuł</label><input type=\'text\' for=\'tyt\' name="tyt[]" /></div>' + "\n";
		 ...
		 div += '<label for=\'skip\'>pomiń </label>  <input type=\'checkbox\' name="skip['+plik+']" id="skip" /></div>' + "\n";
		 div += '</div>' + "\n";

   	div = $j(div);

	div.insertBefore(divThumbs.firstChild);
