...
class Dlp2015b
{
 ...
 private $ended = '2015-09-30';		//Koniec rywalizacji + 1 dzień.
 private $endmonth = 9;
 ...
 function __construct()
 {
  ...
  //	Test::trace(__METHOD__ .' ended1  -> ', C::get('datetime_teraz'));
  ...
  //Test::trace(__METHOD__ .' ended  -> ', $this->ended);
 ...
 */
 ...
 {      //$this->tabela($t, $zw, $pilot, $ob_pkt, $sum_pkt, $id);
  ...
  if($this->wybMonth())
	 $w = '
	<h2>Dolnośląska Liga Paralotniowa '.$this->year.' - '.$this->wybMonth();
  else
   $w = '
	<h2>Dolnośląska Liga Paralotniowa '.$this->year;


  if($this->ended) $w .= ' - Zakończona!';

	$this->w = $w.'</h2>'.$this->menu().$this->w;
