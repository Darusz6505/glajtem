 ...
 */
 ...
 private $kolory = array('#000', '#CC0000', '#0000CC', '#009900', '#FF0000', '#CC6600', '#CC3399', '#336666', '#999900','#000', '#CC0000', '#0000CC', '#009900', '#FF0000', '#CC6600', '#CC3399', '#336666', '#999900', '#000', '#CC0000', '#0000CC', '#009900', '#FF0000', '#CC6600', '#CC3399', '#336666', '#999900');
 ...
 */
 ...
 function __construct()
 {
  ...
  //if(!$this->jo) exit("W trakcie serwisu!");
	/*
  ...
  C::add('javascript_down', '
   ...
   ');
	...
	//src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAcnDqpNWS3VdhDZ50Znsw9r93scGadfwM&callback=initMap" async defer
 ...
 */
  ...
  // key = AIzaSyAcnDqpNWS3VdhDZ50Znsw9r93scGadfwM
  /*
 ...
 */
 ...
 public function igc_testm()
 {

  if($this->opcja && substr($this->opcja, 0, 1) == 'x')
  {

	$this->opcja = substr($this->opcja, 1);
  ...
  $this->w .= $this->menum();				//-wyłaczenie menu
 ...
 */
 ...
 private function taskTest()
 {


  $task = 'zadanie.tsk';

  $plik = $this->katalog.'/'.$task; 			//-skasowanie poprzedniego pliku
   if(file_exists($plik))
   {
	 if($this->jo) $w .= '<p>Jest plik task!</p>';

    $nazpl = $plik;													//-odczytanie pliku
    $h = fopen($nazpl, 'rb');
    ...
    fclose($h);
	  ...
	  $this->taski_sel = '
  <form name="taski" action="">
   <label for="task1"><input id="task1" onclick="onclickTask(1);" type="radio" name="task" value="x1"> task 1 </label>
	<label for="task2"><input id="task2" onclick="onclickTask(2);" type="radio" name="task" value="x2"> task 2 </label>
	<label for="task3"><input id="task3" onclick="onclickTask(3);" type="radio" name="task" value="x2"> task 3 </label>
	<label for="task4"><input id="task4" onclick="onclickTask(4);" type="radio" name="task" value="x2"> task 4 </label>
	<label for="task5"><input id="task5" onclick="onclickTask(0);" type="radio" name="task" value="x2"> task off </label>
  </form>';
	 ...
	 return '<p>Brak Pliku TASK</p>';

   if(!$h) return '<p>Plik pusty!</p>';
	...
	$tr = preg_replace('# {1,}#si', '', $tr);

	$tr = preg_replace('/(\r|\n){2,}/si', '|', $tr);

   if(substr($tr, 0, 1) == '|') $tr = substr($tr, 1);
	if(substr($tr, -1) == '|') $tr = substr($tr, 0, -1);

	$w .= $tr;

	$tr = explode(';', $tr); //-pojedyncze taski

	$taski = $pz = $ts = array();

	foreach($tr as $v1)
	{
	 if(substr($v1, 0, 1) == '|') $v1 = substr($v1, 1);
 ...
 */
 ...
 private function task()
 {
  ...
  if(!$t = $this->taskTest()) return;

  Test::trace('taski t', $t);
  ...
  foreach($t as $val)
  {
   ...
   foreach($val as $war)
   {
		...
		//$al1 = $war[0]; //substr($war, 7, 7);		//-szerokość geograficzna xx-xx.xxxx'
		$al1a = $war[0]; //substr($al1, 0, 2);		//-stopnie
		$al1b = $war[1]; //substr($al1, 2, 2);		//-minuty
		//$al1c = $war[3]; //substr($al1, 4, 3);		//-sekundy

		$al1d = $al1a.'.'.substr((($al1b*1)/60), 2);

		//$al2 = $war[3]; //substr($war, 15, 8);		//-długość geograficzna xx-xx.xxxx'
		$al2a = $war[2]; //substr($al2, 0, 3);
		$al2b = $war[3]; //substr($al2, 3, 2);
		//$al2c = $war[7]; //substr($al2, 5, 3);

		$al2d = $al2a.'.'.substr((($al2b*1)/60), 2);

		$li++;

      $tt[$li] = array('['.$al1d.','.$al2d.']', '['.$war[4].','.$licz.']', $war[5]);
	 }



    foreach($tt as $wart) // pzt'y  tasku
	 {
	  $task[] = '
	 {"type":"task","start":'.$wart[0].',"end":'.$wart[1].',
	 "pilot":"'.$wart[2].'",
	 "points":['.$wart[0].']},
	 {"type":"tsp","start":'.$wart[0].',"end":'.$wart[1].',
	 "pilot":"'.$wart[2].'",
	 "points":['.$wart[0].']}';
  ...
  if($this->pliki_igc())											// odczytanie plików igc z ustalonego katalogu
  {
   ...
   $this->lot = 'var flightPoints = [';						//-nagłówek nowego pliku
    ...
    $this->do_pliku('Początek pliku: '.$plik);
	 ...
	 $this->do_pliku($plik, TRUE);


   $this->w .= $this->konwersja_igc();							//-operacje na plikach igc

   if(count($this->tab_lot) > 0 )												// jeśli jest tablica z lotami, to utworzenie danych dla map.js
   {

	 if($pzy = $this->task())
	  $loty = $pzy . ',' . implode(',', $this->tab_lot);
	 else
	  $loty = implode(',', $this->tab_lot);

      $ww = '
		<div id="taski">'.$this->taski_sel.'</div>
		<div id="loty" ></div>
  ...
  if($this->pliki)
  {
   ...
   $po = 0;
	...
	foreach($this->pliki as $war)
	{
	 ...
	 if($this->jo)
	  $this->analiza_igc($path, $war[0], $po);
	 else
	  $w .= $this->analiza_igc($path, $war[0], $po);
	 ...
	 if($this->lot)
	 {
	  if($this->jo)
	   ...
	   $this->do_pliku($war[0], TRUE);		//-komunikaty wyłączone
    }
	}
 ...
 */
 ...
 private function analiza_igc($path, $name = '??', $p = 0)
 {
  $d_time = $ttmp = 0;
  ...
  if($this->jo)
   ...
   if($tr)
    ...
    $this->lot = '
	  {"type":"flight","start":['.$td[1][0].','.$td[1][1].'],"end":['.$td[$li][0].','.$td[$li][1].'],"kol":"'.$this->kolory[$p].'",
		"pilot":"'.$info.'",
		"points":['.$ttt.']}';
	 ...
	 //array_push($this->tab_lot, $this->lot); // 2021-01-15
 ...
 */
 ...
 /**
 *
 * menu dla katalogu lotów igc
 *
 */

 private function menum()
 {

  $path = $this->katmein;			//-katalog z katalogami lotów

  if(!is_dir($path) && defined($path))
  {
   $this->w = C::infoBox('adm_errbox', 'Brak katalogu z katalogami lotów : '.$path);
   return 0;
  }
  else
  {
	 ...
	 foreach($dirs as $dir)
	 {
	  ...
	  if(file_exists($fils[0]))
	  {
		if(($d = basename($dir)) == $this->opcja)
		{
		 $act = ' igcact';

       $st = $this->katalog = 'igc/'.$this->opcja;
		}
		else
		 $act = '';

	   $w .= '<a class=\'migc'.$act.'\' href=\''.$d.'+'.$this->akcja.'.html\' title=\''.count($fils).' lotów\'>'.basename($dir).'</a>';

		if($act)
		 $w .= '<a class=\'migc'.$act.'\' href=\'x'.$d.'+'.$this->akcja.'.html\' title=\''.count($fils).' lotów\'>terma dla '.basename($dir).'</a>';

		//if(!$ferst) $ferst = $d; //-jeśli brak wyboru, to pierwszy nie pusty katalog

		$ferst = $d; //-ostatni nie pusty katalog jeśli brak wyboru
	  }
	  else
	  {
		$w .= '<span class=\'migc\' title=\'Brak lotów\'>'.basename($dir).'</span>';
	  }
  ...
  {
   if(!$st)
	 $this->katalog = 'igc/'.$ferst;	//-jeśli brak wyboru, to pierwszy nie pusty katalog


   return '
	<div class=\'menudown menu width\'><h4>Wybierz grupę lotów:</h4>'.$w.'
	</div>';
     ...
     if(file_exists($dir))
	  {
	   ++$lf;
		...
		$this->pliki[$lf] = array(basename($dir), filesize($dir));	//-lista plików do analizy
	  }
	 }
