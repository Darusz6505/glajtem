...
class Dlp2017
{
 ...
 private $years = array('2015','2016','2017','2018');
 ...
 function __construct()
 {
  ...
  if(!$opccd[2] = array_intersect($opcc, array('2015', '2016', '2017', '2018')))
   $opccd[2] = $rokNow;
  else
  ...
  //Test::trace(__METHOD__ .' tab1 ->', $tab);
	 ...
	 if($this->nrMonth)
	 {
	  $war2 = ' AND dpl_data < \''. date("Y-m-d", mktime(0, 0, 0, ($this->nrMonth + 1), 1, $this->year)).'\'';
 	  $war2 .= ' AND dpl_data > \''. date("Y-m-d", mktime(0, 0, 0, 2, 28, $this->year)).'\'';
	 }
	 else
	 {
 ...
 {       //$this->tabela($t, $zw, $pilot, $ob_pkt, $sum_pkt, $id);
  ...
  foreach($klas as $k => $v)														// pętla po klasyfikacji, pilotach którzy zgłosili swoje loty
  {
	...
	if($this->nrMonth && $this->nrMonth <= $this->endmonth)
   {
	...
	$row = '
	  <td>&nbsp;'.($lp++).'&nbsp;</td><td class=\'pil_name\'>'.$name.$link_ed_pilot.'</td><td>&nbsp;'.$pilot[$k][1].'&nbsp;</td><td class=\'suma\'><a href=\''.$pilot[$k][3].'+'.$this->wrok.'+pilot.html\' title=\'zobacz listę lotów pilota zakwalifikowanych do DLP\'>'.$sum_pkt[$k].'</a></td><td>'.$lzw.'</td>'.$p;
  ...
  if($this->klase)
   $akcja_back = $this->klase.'+';
	...
	if($this->akcja == 'pilot')
	{
	 ...
	 $this->w =
	'<ul id=\'dlp_menu\'>
	  <li><a '.$class2015.'href=\''.$this->pilot.'+2015+'.$this->akcja.'.html\'> 2015 </a></li>
	  <li><a '.$class2016.'href=\''.$this->pilot.'+2016+'.$this->akcja.'.html\'> 2016 </a></li>
	</ul>'.$w.'</h2>'; */
	}

	//private $stat = array('licz_dni_lot' => -1, 'licz_lot' => -1, 'licz_pil' => -1);
 ...
 */
 ...
 private function sponsor()
 {
  ...
  <div id=\'sponsor\'>
	...
	<img class=\'sponsor_icon\' src="./application/skin/m_tryfly.png" alt=\'TryFly Extreme Shop\'/>
   <!--  <img class=\'sponsor_icon\' src="./application/skin/m_gradient.png" alt=\'SkyTrakking - dystrybutor marki Gradient\'/> -->
	<img class=\'sponsor_icon\' src="./application/skin/m_skytrekking3.png" alt=\'SkyTrakking - szkoła - sklep - serwis\'/>
	<img class=\'sponsor_icon\' src="./application/skin/flylite_logo_pion.png" alt=\'flylite - sklep paralotniowy Pawła Farona\'/>
  </div>';
